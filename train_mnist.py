...
def train_model(model, optimizer, train_loader, test_loader, num_epochs=5):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_features, batch_labels in train_loader:
            ...
            requires_closure = hasattr(optimizer, 'requires_closure') and optimizer.requires_closure

            if requires_closure or isinstance(optimizer, (AdamWithNewtonReset,)):
                def closure():
                    optimizer.zero_grad()
...
n_values = [20, 30, 40, 50, 75, 100, 150]
L_value = 1.0  
beta_newton = 0.15  
...
model_orig = SimpleDNN().to(device)
...
save_model(model_adam, "adam_model_mnist.pth")
save_model(model_sgd, "sgd_model_mnist.pth")
save_model(best_model, f"best_hybrid_model_mnist.pth")
